...
Mat cvcloud_load()
{
//    Mesh mesh = Mesh::load("/Users/zhoushiwei/opencv/samples/cpp/tutorial_code/viz/bunny.ply");
...
//    viz.showWidget("mesha", WMesh(mesh), pose);
//    viz.setRenderingProperty("mesha", SHADING, SHADING_PHONG);
//    viz.showWidget("text2d", WText("Random color mesh", Point(20, 20), 20, Color::green()));
//    viz.spin();
//    waitKey();
//    string filepathOBJ="/Users/zhoushiwei/Dropbox/Viz/shape_0.1.obj";
...
//    EOBJ.extractOBJdata(filepathOBJ, positions, texels, normals, faces_vertex,faces_texel,faces_normal);
//    for (int i=0; i<faces_vertex.size(); i++) {
//        cv::Point3f v1, v2, v3;
//        int a=faces_vertex.at(i).x;
//        int b=faces_vertex.at(i).y;
//        int c=faces_vertex.at(i).z;
//        
//        v1=positions.at(a);
//        v2=positions.at(b);
//        v3=positions.at(c);
//        cv::Point3f normal=getNormal(v1,v2,v3);
//        faces_normal_vertex.push_back(normal);
//    }
    Mat cloud(1, 11510, CV_32FC3);
    ifstream ifs("/Users/zhoushiwei/Dropbox/Viz/shape_0.1.obj");
    
    Point3f* data = cloud.ptr<cv::Point3f>();
    Point3f* normaldata = cloud.ptr<cv::Point3f>();
    float dummy1, dummy2;
    string ss;
    for(size_t i = 0; i < 11510; ++i){
        ifs>>ss;
    ...
    for(size_t i = 0; i < 11510; ++i){
        ifs>>ss;
      //  ifs >> normaldata[i].x >> normaldata[i].y >> normaldata[i].z;
    }
...
//        }
        ...
        for (int i=0; i<faces_vertex.size(); i++) {
          ...
          //  ifss<<"vn "<<normal.x<<" "<<normal.y<<" "<<normal.z<<endl;
        }
        for (int i=0; i<normals.size(); i++) {
             ifss<<"vn "<<normals.at(i).x<<" "<<normals.at(i).y<<" "<<normals.at(i).z<<endl;
        }
        ...
        for (int i=0; i<faces_vertex.size(); i++) {
           ...
           ifss<<"f "<<faces_vertex.at(i).x+1<<"/"<<faces_vertex.at(i).x+1<<"/"<<faces_texel.at(i).x+1<<" "<<faces_vertex.at(i).y+1<<"/"<<faces_vertex.at(i).y+1<<"/"<<faces_texel.at(i).y+1<<" "<<faces_vertex.at(i).z+1<<"/"<<faces_vertex.at(i).z+1<<"/"<<faces_texel.at(i).z+1<<endl;
